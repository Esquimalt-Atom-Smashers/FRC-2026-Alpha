...
public class RobotContainer {
      ...
      // Real robot, instantiate hardware IO implementations
		...
		safeRetractExtenderCommand =
				SafeRetractExtenderCommand.create(
						shootWhenReadyCommand, flywheel, extender, intake, turret, b -> driverTurretOverride = b);
  ...
  private void configureFuelSim() {
	...
	private static Pose3d[] buildComponentPoses(Turret turret, Extender extender, Hang hang, Hood hood) {
		return new Pose3d[] {
			...
			new Pose3d(-0.095, -0.09, 0.41, new Rotation3d(hood.getAngleRad() - Math.toRadians(90), 0, turret.getRobotFramePosition().getRadians() - Math.toRadians(90)))
		};
  ...
  /** Update the Simulation world. Should be called from Robot.simulationPeriodic(). Only works in SIM mode. */
	...
	private void finishSecondSimRobotSetup(SecondSimRobotBundle secondSimRobotBundle) {
		...
		secondSimRobotBundle.safeRetractExtenderCommand =
				SafeRetractExtenderCommand.create(
						secondSimRobotBundle.shootWhenReady, secondSimRobotBundle.flywheel, secondSimRobotBundle.extender, secondSimRobotBundle.intake, secondSimRobotBundle.turret,
