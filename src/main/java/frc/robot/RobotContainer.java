...
import edu.wpi.first.math.geometry.ChassisSpeeds;
...
public class RobotContainer {
    ...
    switch (Constants.currentMode) {
      case REAL:
        ...
        if (drive != null) {
          ...
          vision = new Vision((pose, timestamp) -> {}, new VisionIOPhotonVision(camera0Name, robotToCamera0), new VisionIOPhotonVision(camera1Name, robotToCamera1));
        }
        break;
  ...
  private void configureDriveBindings(boolean enableDriving) {
    ...
    if (enableDriving) {
      ...
      drive.setDefaultCommand(
          Commands.run(
              () -> {
                ...
                double velocityX = scaleAxisWithTurbo(-driverController.getLeftY(), driverController.getRightTriggerAxis(), MAX_CONTROL_SPEED);
                double velocityY = scaleAxisWithTurbo(-driverController.getLeftX(), driverController.getRightTriggerAxis(), MAX_CONTROL_SPEED);
                double rotationalRate = scaleAxisWithTurbo(-driverController.getRightX(), driverController.getRightTriggerAxis(), MAX_ANGULAR_RATE);
      ...
      driverController.start().onTrue(Commands.runOnce(() -> drive.setPose(new Pose2d(drive.getPose().getTranslation(), Rotation2d.kZero)), drive));
